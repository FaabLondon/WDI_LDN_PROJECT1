...
It features 5 levels with different game speed and enemies. The 2 first levels are an introduction to the game, its enemies and Mario moves, while the last 3 levels have an extra challenge as Mario needs to catch a certain number of coins in order to progress to the next level.
...
###### In level 3 , Mario needs to catch at least 30 coins within 45 seconds to unlock the next level. Once the 30 coins are caught, the player hears a "yay" from Mario to indicate the goal has been reached. Mario still need to finish the level while avoiding the enemies. If Mario doesn't reach 30 coins he loses 1 life and needs to try the level again. In that level a new enemy (evil turtle) appears and the coins drop faster.
